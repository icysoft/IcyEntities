...
var errorsManager = errors.manager;
var errorsCodes = errors.codes;
...
EntityManager.prototype.getBucket = function () {
    throw errorsManager.ERROR(errorsCodes.bucketNotSet);
};
 ...
 * @param {Function} callback
 */
EntityManager.prototype.upsert = function (entity, options, callback) {
    if (options instanceof Function) {
        callback = arguments[1];
        options = {};
    }
    ...
    if (entity.uid === 0) {
        this.generateUid(entity, function (err, result) {
            if (err) {
                ...
                entity.uid = result;
                return this.upsert(entity, options, callback);
            }
        }.bind(this));
    }
    else {
        ...
        this.bucket.upsert(entity.uid, data, options, function (err, result) {
            if (err) {
                return callback(err);
            }
            entity.cas = result.cas;
...
EntityManager.prototype.getData = function (entity) {
    ...
    return errorsManager.ERROR(errorsCodes.schemaMismatch, this.validate.errors, 'BDD: Schema incompatible');
};
...
EntityManager.prototype.assertType = function (data) {
    ...
    return errorsManager.ERROR(errorsCodes.typeMismatch, {required: this.type, current: data.type}, 'BDD: Type incompatible');
};
